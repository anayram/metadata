<?xml version="1.0"?>
<!-- Stylesheet to create reports from Peel mods metadata -->
<!--
    ...
    <xsl:template name="url">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$genre[contains(.,'newspaper')]">
                <xsl:value-of select="concat($peelsite,'newspapers/',$nid,'.html')"/>
            </xsl:when>
            ...
            <xsl:when test="$id[starts-with(.,'P0')]">
                <xsl:value-of select="concat($peelsite,'bibliography/',replace($id,'P\d\d(.*)','$1'),'.html')"/>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
